...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------
    # 实验 B：对比正则化参数 (Weight Decay)
    # 锁定参数：Epoch=5, 我们取上面跑出来通常最稳的学习率 lr=1e-4
    # ----------------------------------------------------
    wd_list = [0.0, 1e-4, 1e-3, 1e-2] # 分别对应：无正则、弱正则、强正则
    for wd in wd_list:
        run_experiment(
    ...
    # ----------------------------------------------------
    # 实验 C：对比训练步数 (Epochs)
    # 锁定参数：lr=1e-4, weight_decay=1e-4
    # ----------------------------------------------------
    epoch_list = [5, 10, 15] # 跑一个 5 轮的，再跑一个 10 轮的对比长度
    ...
    print("====== 🏆 所有实验全部跑完！快去 Swanlab 收割截图吧！ ======")